...
export const shlokSlice = createSlice({
    ...
    reducers: {
        ...
        addFavorite: (state, action: PayloadAction<Shlok>) => {
             const shlok = action.payload;
             if (!state.favorites.some(s => s.chapter === shlok.chapter && s.verse === shlok.verse)) {
                 state.favorites.push(shlok);
                 localStorage.setItem('vedincharya_state', JSON.stringify(state));
             }
        },
        removeFavorite: (state, action: PayloadAction<{ chapter: number; verse: number }>) => {
            state.favorites = state.favorites.filter(
                s => !(s.chapter === action.payload.chapter && s.verse === action.payload.verse)
            );
            localStorage.setItem('vedincharya_state', JSON.stringify(state));
        },
...
export const { toggleFavorite, addToRecent, updateSettings, addFavorite, removeFavorite, clearFavorites } = shlokSlice.actions;
